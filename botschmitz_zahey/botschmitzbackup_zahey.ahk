...
char:= "backup"
#Include %A_ScriptDir%\botschmitzcfg_zahey.ini
...
gosub openmapslave
sleep 1000
random,randX, 405, 406
random,randY, 120, 121
...

{
				...
				sleep 1000 ; killtime
				ControlClick, x370 y305, %Title3%,, LEFT, 1, NA ; kill
				sleep 3000
				goto Find
}
	...
	Loop, parse, Console, `n, `r  ; Specifying `n prior to `r allows both Windows and Unix files to be parsed.
{
	...
	controlsettext, Edit1, %TimeString% - %msg%`r`n%str%, Client Fern ahk_class AutoHotkeyGUI
	sendError(msg)
...
movetilstop:
ControlClick, x423 y85, %Title3%,, LEFT, 1, NA ;location far btm right
sleep 10000
loop, {
	cou++
ControlClick, x423 y85, %Title3%,, LEFT, 1, NA ;location far btm right
