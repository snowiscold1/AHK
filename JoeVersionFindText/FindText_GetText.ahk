...
FindText() approaches the search differently than built-in ImageSearch. Think 1980's ASCII art. This function abstracts the screen's image into representative 0's and _'s.
  ...
  3. When test is successful, you may copy the code and paste it into your own script.
     Note: Copy the "FindText()" function and the following functions and paste it into your own script Just once.
...
There two comparison modes: color and greyscale. You can choose either. These are slightly different ways to determine if each pixel color/greyscale in the target image becomes simplified to black or white and eventually a "0" or "_". The threshold is the divide between black and white determination. Threshold too low -> all white, Threshold to high -> all black and at these extremes matching accuracy is low.
 ...
 For fun you can click the long Text string in the lower pane, and the upper window will show the corresponding ASCIIart searchtext
===========================================
  ...
  returnArray := FindText( center point X, center point Y
    ...
    , Character "_" fault-tolerant in percentage, text ) ; "_" is background charachter.
;~  Note: when the similarity is less than 1, the tolerance of the background character (_) should be a little larger, in some cases it can even be set to 1.


  parameters of the X,Y is the center of the coordinates, and the W,H is the offset distance to the center,
  So the search range is (X-W, Y-H)-->(X+W, Y+H).

  The fault-tolerant parameters allow the loss of specific characters, very useful for gray threshold model.
