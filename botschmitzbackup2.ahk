...
{
	sleep 1000
	ControlClick, x477 y52, %Title4%,, LEFT, 1, NA
}
gosub openmapslave
...
while(!ok:=FindText(GHhallSlave[1], GHhallSlave[2], GHhallSlave[3], GHhallSlave[4], 0, 0, GHhallSlave[5]))
{
	...
	if (WaitingTime=3) {
				...
				Tooltip, 
				WaitingTime=0
	ControlClick, x334 y77, %Title4%,, LEFT, 1, NA ;chgechatmap
	sleep 1000
EnterLine:
log(Title4 " is looking for available channel ...")
FindText_BindWindow(Title4ID)
 ...
 }
 {
				...
				WaitingTime=0
				sleep 500
				ControlClick, x440 y100, %Title4%,, LEFT, 1, NA  ; enterline
 }
...
sleep 500
ControlClick, x496 y306, %Title4%,, LEFT, 1, NA
sleep 500
ControlClick, x239 y245, %Title4%,, LEFT, 1, NA ; open country list
sleep 500
FindText_BindWindow(Title4ID)
...
{
	c++
	ControlClick, x382 y303, %Title4%,, LEFT, 1, NA
	sleep 1000
	if (c>4) {
		c=0
		runonce++
...
if(ok:=FindText(dontremind[1], dontremind[2], dontremind[3], dontremind[4], 0, 0, dontremind[5]))
{
				...
				ControlClick, x%x% y%y%, %Title4%,, LEFT, 1, NA
				sleep 500
				ControlClick, x318 y259, %Title4%,, LEFT, 1, NA
}
...
ControlClick, x%randX% y%randY%, %Title4%,, LEFT, 1, NA ;location spot btm right
...
random,randY, %spawnY%, %spawny%+1
				...
				nomob++
				jumpa = 0
				log(Title4 " is unable to find King Schmitz at " CountNumb " ...")
				gosub openmapslave
				goto Find
				}
}

{
				...
				jumpa = 1
				totalkill++
				log(Title4 " has found King Schmitz at " CountNumb " ...")
				gosub movetilstop
				DetectHiddenWindows, On
				SetTitleMatchMode, 2
				FileAppend, %CountNumb%`n, %A_ScriptDir%\logfoundbackup.txt
				;sleep 1000 ; killtime
				;ControlClick, x370 y305, %Title4%,, LEFT, 1, NA ; kill
				;sleep 3000
				goto Find
}
...
{
				ControlClick, x511 y270, %Title4%,, LEFT, 1, NA
				sleep 2000
}
FindText_BindWindow(Title4ID)
...
{	
			...
			ControlClick, x%randX% y%randY%, %Title4%,, LEFT, 1, NA
			sleep 1000
			nomob = 0
			jumpa = 1
			totalkill++
...
While (!ok:=FindText(ghostcapavatar[1],ghostcapavatar[2],ghostcapavatar[3],ghostcapavatar[4], 0, 0, ghostcapavatar[5]))
				{
					c++
					ControlClick, x510 y243, %Title4%,, LEFT, 1, NA
					sleep 300
					if (c>6) {
						c=0
						return
...
log(Title4 " has found Ghost Cap at " CountNumb "...")
return
...
;ControlClick, x509 y267, %Title4%,, LEFT, 1, NA
...
while(ok:=FindText(enterhall[1], enterhall[2], enterhall[3], enterhall[4], 0, 0, enterhall[5]))
{
	ControlClick, x352 y301, %Title4%,, LEFT, 1, NA ; enterguild
	sleep 1000
}
sleep 20000
gosub openmapslave
...
while(ok:=FindText(leaveguild[1], leaveguild[2], leaveguild[3], leaveguild[4], 0, 0, leaveguild[5]))
{
	ControlClick, x318 y238, %Title4%,, LEFT, 1, NA ;leave
	sleep 1000
}
...
while(ok:=FindText(confirmleave[1], confirmleave[2], confirmleave[3], confirmleave[4], 0, 0, confirmleave[5]))
{
	ControlClick, x315 y170, %Title4%,, LEFT, 1, NA ; confirm leave
	sleep 1000
}
...
{	
ControlClick, x498 y75, %Title4%,, LEFT, 1, NA
sleep 1000
}
return
...
movetilstop:
ControlClick, %tauntcoord1x% %tauntcoord1y%, %Title4%,, LEFT, 1, NA ;location far btm right
sleep 10000
loop, {
	cou++
ControlClick, %tauntcoord1x% %tauntcoord1y%, %Title4%,, LEFT, 1, NA ;locationfar btm right
  pic1:=GetPicArr(1229,604,20,20)
  Sleep, 1000
  pic2:=GetPicArr(1229,604,20,20)
  if (!ComparePicArr(pic1, pic2, n:=0.001)) {
