...
if(ok:=FindText(GlastheimAreaNurse[1],GlastheimAreaNurse[2],GlastheimAreaNurse[3],GlastheimAreaNurse[4], 0.1, 0.1, GlastheimAreaNurse[5]))
{
		CoordMode, Mouse
		X:=ok.1.x-(830), Y:=ok.1.y, Comment:=ok.1.id
		ControlClick, x%x% y%y%, %Title2%,, LEFT, 1, NA
...
while(!ok:=FindText(CulvertNurse[1],CulvertNurse[2],CulvertNurse[3,CulvertNurse[4]], 0.1, 0, CulvertNurse[5]))
		{
			SetTimer, MaxTime, 1000
			sleep 1000
				if (WaitingTime=20) {
